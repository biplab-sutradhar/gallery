...
test_that("Generated data frame dimensions are consistent", {
  expect_true(ncol(full_data) == 3)
  expect_true(ncol(time_data) == 2)
})

test_that("Generated time intervals are correct", {
  expect_true(all(diff(time_data$iteration) == 1))
  expect_true(all(!is.na(time_data$main)))
})

test_that("Visualization title and labels are correct", {
  expect_true(grepl("Frequency of Stock Prices", saveXML(html_content)))
  expect_true(grepl("Price", saveXML(html_content)))
  expect_true(grepl("Frequency", saveXML(html_content)))
})

test_that("Animation steps are correctly defined", {
  expect_true(grepl("iteration", saveXML(html_content)))
})
